...
import { fetchRatingAgencyMonthlyDetailedData } from '@/features/ratingAgencies/services';
...
const defaultColumns: string[] = allColumns.map(c => c.accessor);
...
export default function IssuerListPage() {
    ...
    const fetchData = useCallback(async () => {
        ...
        try {
            ...
            const result: PaginatedApiResponse = await fetchRatingAgencyMonthlyDetailedData(requestBody);
            ...
            return (
                <span
                    className={`
                        ...
                        ${!['Equity', 'Debentures', 'Mutual Fund', 'Hybrid Fund'].includes(String(value)) ? 'bg-gray-100 text-gray-700 dark:bg-gray-800 dark:text-gray-300' : ''}
                    `}
                >
                    {value}
                </span>
    ...
    const handleExport = useCallback(() => {
        ...
        const exportData = sortedData.map((row) => {
            ...
            filteredColumns.forEach(col => {
                ...
                if (col.accessor === 'issueSize' || col.accessor === 'faceValue') {
                    newRow[col.header] = (value === null || value === undefined || value === 0) ? '-' : formatCurrency(value);
                } else {
        ...
        XLSX.utils.book_append_sheet(workbook, worksheet, 'Rating Agencies List');

        // Generate filename with current date
        const dateStr = new Date().toISOString().split('T')[0];
        const filename = `rating-agencies-list-${fy}-${period}-${dateStr}.xlsx`;
        ...
        <SkeletonTheme enableAnimation={true} baseColor="#1F2937" highlightColor="#90969bff" borderRadius="0.5rem">
            <div className="min-h-full space-y-4 font-sans text-gray-700 dark:text-gray-200 p-4">
                ...
                <div className="flex flex-col items-start gap-3">
                    ...
                    <div>
                        <h1 className="text-xl font-bold">Rating Agencies List</h1>
                        <p className="text-xs text-gray-500 mt-1 mb-6">Rating Agency &gt; List</p>
                    </div>
                </div>
                        ...
                        <div className="flex items-center gap-2 w-full sm:w-auto">
                            ...
                            <div className="relative" ref={dropdownRef}>
                                <button
                                    ...
                                    className="text-xs border border-gray-300 dark:border-gray-600 px-3 h-8 rounded-md bg-white dark:bg-[#1a1a2e] text-gray-700 dark:text-gray-200 hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors"
                                >
                                    Columns
                                </button>
                                        ...
                                        {sortedData.map((row, index) => (
                                            ...
                                            >
                                                {filteredColumns.map((column) => (
                                                    <td
                                                        key={column.accessor}
                                                        className={`border border-gray-200 dark:border-gray-700 rounded-md px-3 py-3 font-medium break-words min-w-[120px] text-gray-800 dark:text-gray-200 ${column.accessor === 'issueSize' || column.accessor === 'faceValue' ? 'text-right' : ''
                                                            }`}
                                                    >
