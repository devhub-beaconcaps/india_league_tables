...
import { fetchArrangerTopParticipantsData } from '@/features/arrangers/services';
...
export default function ArrangerTopParticipantsPage() {
    ...
    const apiSortFieldMap: Record<string, string> = {
        ...
        securityType: 'security_name',
        modeOfIssue: 'issuer_name',
        ...
        debentureTrustee: 'issuer_name',
        registrar: 'issuer_name',
        arranger: 'arranger_name',
        seniority: 'issuer_name',
        taxFree: 'issuer_name',
        securedFlag: 'issuer_name',
    ...
    const fetchData = useCallback(async () => {
        if (!dateRange || !arrangerId) return;
        ...
        try {
            ...
            console.error('Error fetching arranger top participants:', err);
    ...
    const handleExport = () => {
        ...
        try {
            const headers = [
                ...
                'Issuer Name',
                'ISIN',
                ...TABLE_COLUMNS.filter((col) => col.key !== 'arranger').map((col) => col.label),
            ];
            ...
            const dataRows = tableData.map((row) => [
                ...
                row.issuerName,
                row.isin,
                ...TABLE_COLUMNS.filter((col) => col.key !== 'arranger').map((col) => {
            ...
            const now = new Date();
            const filename = `arranger-top-participants-${now.getFullYear()}-${(now.getMonth() + 1)
                .toString()
    ...
    return (
        <SkeletonTheme enableAnimation={true} baseColor="#1F2937" highlightColor="#90969bff" borderRadius="0.5rem">
            <div className="min-h-full space-y-4 font-sans text-gray-700 dark:text-gray-200 p-4">
                {/* Header */}
                <div>
                    <h1 className="text-xl font-bold text-gray-700 dark:text-gray-200">Arranger Top Participants</h1>
                    <p className="text-[9px] text-gray-400 mb-6 mt-1">Arranger &gt; Top Participants</p>
                </div>
                ...
                <div className="bg-white dark:bg-[#1a1a2e] rounded-[12px] shadow-sm border border-gray-200 dark:border-gray-600 p-5 space-y-5">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                        <div className="flex flex-col gap-1">
                            <label className="text-[9px] text-gray-400">Arranger ID</label>
                    ...
                    {!arrangerId && (
                        <p className="text-[9px] text-red-500 pt-2">Arranger ID is missing from URL parameters.</p>
                    )}
                </div>
                                ...
                                <table className="w-full table-auto border-separate border-spacing-[4px] text-[12px]">
                                    <thead>
                                        <tr className="bg-gradient-to-r from-[#423CAB] to-[#653FD8] text-white">
                                            ...
                                            {TABLE_COLUMNS.filter((column) => column.key !== 'arranger').map((column) => (
                                                <th
                                                    key={column.key}
