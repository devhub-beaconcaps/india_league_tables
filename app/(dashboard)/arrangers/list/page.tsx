...
import { fetchArrangerMonthlyDetailedData } from '@/features/arrangers/services';
...
export default function IssuerListPage() {
    ...
    const fetchData = useCallback(async () => {
        ...
        try {
            ...
            const result: PaginatedApiResponse = await fetchArrangerMonthlyDetailedData(requestBody);
    ...
    const handleExport = () => {
        ...
        try {
            // Prepare headers - Arranger as first column, then other table columns
            const headers = ['Arranger', ...TABLE_COLUMNS.filter(col => col.key !== 'arranger').map(col => col.label)];
            ...
            XLSX.utils.book_append_sheet(workbook, worksheet, 'Arranger List');

            // Generate filename with current date
            const now = new Date();
            const filename = `arranger-list-${now.getFullYear()}-${(now.getMonth() + 1).toString().padStart(2, '0')}-${now.getDate().toString().padStart(2, '0')}.xlsx`;
    ...
    return (
        <SkeletonTheme enableAnimation={true} baseColor="#1F2937" highlightColor="#90969bff" borderRadius="0.5rem">
            <div className="min-h-full space-y-4 font-sans text-gray-700 dark:text-gray-200 p-4">
                {/* Header */}
                <div>
                    <h1 className="text-xl font-bold text-gray-700 dark:text-gray-200">Arrangers List</h1>
                    <p className="text-[9px] text-gray-400 mb-6 mt-1">Arranger &gt; List</p>
                </div>
                                ...
                                <table className="w-full table-auto border-separate border-spacing-[4px] text-[12px]">
                                    <thead>
                                        <tr className="bg-gradient-to-r from-[#423CAB] to-[#653FD8] text-white">
                                            {/* Arranger — moved to first column */}
                                            <th
