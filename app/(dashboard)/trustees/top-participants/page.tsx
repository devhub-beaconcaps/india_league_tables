...
export default function TrusteeTopParticipantsPage() {
    ...
    const fetchData = useCallback(async () => {
        ...
        try {
            ...
            console.error('Error fetching trustee top participants:', err);
    ...
    const handleExport = () => {
        ...
        try {
            const headers = [
                ...
                'Issuer Name',
                'ISIN',
                ...TABLE_COLUMNS.filter((col) => col.key !== 'debentureTrustee').map((col) => col.label),
            ];
            ...
            const dataRows = tableData.map((row) => [
                ...
                row.issuerName,
                row.isin,
                ...TABLE_COLUMNS.filter((col) => col.key !== 'debentureTrustee').map((col) => {
            ...
            const now = new Date();
            const filename = `trustee-top-participants-${now.getFullYear()}-${(now.getMonth() + 1)
                .toString()
                ...
                <div className="bg-white dark:bg-[#1a1a2e] rounded-[12px] shadow-sm border border-gray-200 dark:border-gray-600 p-5 space-y-5">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                        <div className="flex flex-col gap-1">
                            <label className="text-[9px] text-gray-400">Trustee ID</label>
                    ...
                    {!trusteeId && (
                        <p className="text-[9px] text-red-500 pt-2">Trustee ID is missing from URL parameters.</p>
                    )}
                </div>
                                ...
                                <table className="w-full table-auto border-separate border-spacing-[4px] text-[12px]">
                                    <thead>
                                        <tr className="bg-gradient-to-r from-[#423CAB] to-[#653FD8] text-white">
                                            ...
                                            {TABLE_COLUMNS.filter((column) => column.key !== 'debentureTrustee').map((column) => (
                                                <th
                                                    key={column.key}
