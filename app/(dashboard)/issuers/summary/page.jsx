...
const creditAgencyDropdownOptions = creditRatingAgencyOptions.map(item => ({
    label: item.label,
    value: item.id
}));
...
function getMonthDates(month, year) {
    // Convert short month to index
    const monthIndex = new Date(`${month} 1, ${year}`).getMonth();

    const today = new Date();

    // Default start date (1st of month)
    let start = new Date(year, monthIndex, 1);

    // If provided month/year is current month/year
    if (
...
export default function IssuerSummary() {
    ...
    const handleBarClick = (data) => {
        ...
        const { startDate, endDate } = getMonthDates(data.month, data.year);

        setRedemptionMonthDateRange({ startDate, endDate });


        // console.log("Current redemption month range from store: ", redemptionMonthDateRange,"startDate, endDate",startDate, endDate);
        ...
        <div className="min-h-full space-y-4 font-sans text-gray-800 dark:text-gray-100">
            ...
            <SectionCard>
                ...
                <div className="overflow-x-auto">
                    <FinanceTable totalsData={totalsData} data={issueTableData} selectedFY={selectedFY} valueConvention={valueConvention} />
                </div>
            </SectionCard>
                ...
                <div className="flex flex-col items-center justify-center gap-8 flex-wrap">
                    <div className="relative">
                        <ResponsiveContainer width={220} height={220}>
                            <PieChart>
                                <Pie data={ratingData} cx="50%" cy="50%" innerRadius={55} outerRadius={110} dataKey="value" labelLine={false} label={renderLabel} startAngle={90} endAngle={-270}>
