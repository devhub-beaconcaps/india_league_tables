...
export default function RegistrarsMonthWiseSummary() {
    ...
    const fetchPrimaryData = useCallback(async () => {
        try {
            ...
            const res = await fetchRegistrarMonthlySummaryData({
                startDate: primaryStartDate,
                endDate: primaryEndDate,
    ...
    const handleExportMonthWiseCSV = useCallback(() => {
        ...
        link.href = url;
        link.download = `month_wise_registrars_${primaryYearLabel}${enableCompare ? `_vs_${compareYearLabel}` : ''}.csv`;
    ...
    const handleExportQuarterWiseCSV = useCallback(() => {
        ...
        const url = URL.createObjectURL(blob);
        link.href = url;
        link.download = `quarter_wise_registrars_${primaryYearLabel}${enableCompare ? `_vs_${compareYearLabel}` : ''}.csv`;
        document.body.appendChild(link);
        link.click();
        document.body.removeChild(link);
        URL.revokeObjectURL(url);
