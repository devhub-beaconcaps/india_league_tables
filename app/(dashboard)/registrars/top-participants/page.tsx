...
export default function RegistrarTopParticipantsPage() {
    ...
    // URL params: ?id=8&fy=2026-2027&startDate=...&endDate=...&ownershipType=...
    ...
    const fetchData = useCallback(async () => {
        ...
        try {
            ...
            // ── Read filters from URL query params (same pattern as Arrangers page) ──
    ...
    }, [dateRange, registrarId, currentPage, pageSize, searchQuery, sortColumn, sortDirection, getFilterArray]);
    ...
    const handleExport = () => {
        ...
        try {
            const headers = [
                ...
                'Issuer Name',
                'ISIN',
                ...TABLE_COLUMNS.filter((col) => col.key !== 'registrar').map((col) => col.label),
            ];
            ...
            const dataRows = tableData.map((row) => [
                ...
                row.issuerName,
                row.isin,
                ...TABLE_COLUMNS.filter((col) => col.key !== 'registrar').map((col) => {
        ...
        <SkeletonTheme enableAnimation={true} baseColor="#1F2937" highlightColor="#90969bff" borderRadius="0.5rem">
            <div className="min-h-full space-y-4 font-sans text-gray-700 dark:text-gray-200 p-4">
                ...
                <div className="flex flex-col items-start gap-3">
                    ...
                    <div>
                        <h1 className="text-xl font-bold">Registrar Top Participants</h1>
                        <p className="text-xs text-gray-500 mt-1 mb-6">Registrar &gt; Top Participants</p>
                    </div>
                </div>
                                ...
                                <table className="w-full table-auto border-separate border-spacing-[4px] text-[12px]">
                                    <thead>
                                        <tr className="bg-gradient-to-r from-[#423CAB] to-[#653FD8] text-white">
                                            ...
                                            {TABLE_COLUMNS.filter((column) => column.key !== 'registrar').map((column) => (
                                                <th
                                                    key={column.key}
