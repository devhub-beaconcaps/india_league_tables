...
import { fetchRegistrarTopParticipantsData } from '@/features/registrars/services';
...
// Columns rendered after the first 3 fixed columns (Registrar, Issuer Name, ISIN)
const DATA_COLUMNS = TABLE_COLUMNS.filter((col) => col.key !== 'registrar');
...
export default function RegistrarTopParticipantsPage() {
    const router = useRouter();
    const searchParams = useSearchParams();

    // URL params: ?id=8&fy=2026-2027
    const registrarId = searchParams.get('id') || '';
    ...
    const fetchData = useCallback(async () => {
        if (!dateRange || !registrarId) return;
        ...
        try {
            ...
            const result = await fetchRegistrarTopParticipantsData(requestBody);
            ...
            console.error('Error fetching registrar top participants:', err);
    ...
    }, [dateRange, registrarId, currentPage, pageSize, searchQuery, sortColumn, sortDirection]);
    ...
    const handleExport = () => {
        ...
        try {
            ...
            const now = new Date();
            const filename = `registrar-top-participants-${now.getFullYear()}-${(now.getMonth() + 1)
                .toString()
    ...
    return (
        <SkeletonTheme enableAnimation={true} baseColor="#1F2937" highlightColor="#90969bff" borderRadius="0.5rem">
            <div className="min-h-full space-y-4 font-sans text-gray-700 dark:text-gray-200 p-4">
                {/* Header */}
                <div>
                    <h1 className="text-xl font-bold text-gray-700 dark:text-gray-200">Registrar Top Participants</h1>
                    <p className="text-[9px] text-gray-400 mb-6 mt-1">Registrar &gt; Top Participants</p>
                </div>
                ...
                <div className="bg-white dark:bg-[#1a1a2e] rounded-[12px] shadow-sm border border-gray-200 dark:border-gray-600 p-5 space-y-5">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                        <div className="flex flex-col gap-1">
                            <label className="text-[9px] text-gray-400">Registrar ID</label>
                    ...
                    {!registrarId && (
                        <p className="text-[9px] text-red-500 pt-2">Registrar ID is missing from URL parameters.</p>
                    )}
                </div>
                                        ...
                                        {tableData.map((row, index) => (
                                            ...
                                            >
                                                ...
                                                {DATA_COLUMNS.map((column) => {
                                                            ...
                                                            >
                                                                <span
                                                                    className={`
                                                                        ...
                                                                        ${value === 'Municipal Bonds' ? 'bg-teal-100 text-teal-700 dark:bg-teal-900/30 dark:text-teal-400' : ''}
                                                                        ${!['Equity', 'Debentures', 'Mutual Fund', 'Hybrid Fund', 'Municipal Bonds'].includes(value) ? 'bg-gray-100 text-gray-700 dark:bg-gray-800 dark:text-gray-300' : ''}
                                                                    `}
                                                                >
                                                                    {value || '-'}
                                                                </span>
                                                    ...
                                                    if (column.key === 'issueSize' || column.key === 'faceValue') {
                                                        return (
                                                            <td
                                                                key={column.key}
                                                                className="border border-gray-200 dark:border-gray-700 rounded-md px-3 py-3 font-medium whitespace-nowrap min-w-[120px] text-gray-800 dark:text-gray-200 text-right"
                                                            >
