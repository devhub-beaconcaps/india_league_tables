    ...
    return (
        <div className="pl-4 sm:pl-5 pt-4 sm:pt-5 border border-gray-200 dark:border-gray-700 rounded-[13px] bg-white dark:bg-gray-900">
            <h2 className="text-[15px] sm:text-[17px] font-semibold text-gray-800 dark:text-gray-100 mb-4">
                {isLoading ? <Skeleton width={220} /> : title}
            </h2>
...
export default function IssuerProfileCard({
    issuerData,
    isLoading = false,
}: IssuerProfileCardProps) {
    const [activeSection, setActiveSection] =
        useState<SectionId>("issuer-profile");
    ...
    const issuerName =
        issuerData?.find((item) => item.label === "Issuer Name")?.value ?? null;

    const ISINName =
        issuerData?.find((item) => item.label === "ISIN")?.value ?? null;
